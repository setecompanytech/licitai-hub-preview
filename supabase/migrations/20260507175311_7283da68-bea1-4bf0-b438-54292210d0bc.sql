CREATE OR REPLACE FUNCTION public.restaurar_lancamentos_audit(p_limite integer DEFAULT 500)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  WITH alvo AS (
    SELECT al.dados_antes, (al.dados_antes->>'id')::uuid AS lid
    FROM public.financeiro_audit_log al
    WHERE al.tabela='financeiro_lancamentos' AND al.operacao='DELETE'
      AND coalesce(al.dados_antes->>'origem_tipo','') NOT IN ('seed','migracao')
    LIMIT p_limite * 3
  ),
  alvo_filtrado AS (
    SELECT a.dados_antes
    FROM alvo a
    WHERE NOT EXISTS (SELECT 1 FROM public.financeiro_lancamentos l WHERE l.id = a.lid)
    LIMIT p_limite
  ),
  rec AS (
    SELECT (jsonb_populate_record(NULL::public.financeiro_lancamentos, dados_antes)).* FROM alvo_filtrado
  ),
  rec_dedup AS (
    SELECT DISTINCT ON (id) * FROM rec ORDER BY id, updated_at DESC NULLS LAST
  )
