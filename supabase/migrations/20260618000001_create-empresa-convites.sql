...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'empresa_convites' AND policyname = 'admin pode gerenciar convites') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'empresa_convites' AND policyname = 'convite publico por token') THEN
