CREATE OR REPLACE FUNCTION public.restaurar_lancamentos_por_id(p_limite integer DEFAULT 10000)
...
BEGIN
  WITH alvo AS (
    ...
    WHERE al.tabela='financeiro_lancamentos' AND al.operacao='DELETE'
      ...
      AND coalesce(al.dados_antes->>'origem_tipo','') NOT IN ('seed')
  ...
  RETURN v_count;
END $$;
ALTER FUNCTION public.restaurar_lancamentos_por_id(integer) SET statement_timeout='180s';