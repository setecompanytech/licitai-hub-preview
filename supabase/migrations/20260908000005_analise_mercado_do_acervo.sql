-- ═══════════════════════════════════════════════════════════════════════════
-- 20260908000005 · Análise de Mercado passa a ler o ACERVO REAL (08/09)
-- ═══════════════════════════════════════════════════════════════════════════
--
-- A página exibia KPIs e gráficos com números LITERAIS no código ("4.657",
-- "R$ 3,2 bi") e "extrações" que pediam à IA generativa para estimar
-- empenhos — número inventado com cara de coleta, a pior falha silenciosa.
-- O acervo PNCP local (pncp_editais_cache: 18 mil+ editais, semeadura PA
-- completa) tem os dados verdadeiros; este RPC agrega o que a tela mostra.
--
-- Valores: usa o TOTAL ESTIMADO declarado pelo órgão (homologado quando
-- houver seria outra régua). Outliers de digitação (>R$ 10 bi num edital)
-- ficam fora das somas — placeholder de órgão não é volume de mercado.
...
WITH base AS (
  ...
  WHERE data_publicacao_pncp IS NOT NULL
    AND data_publicacao_pncp::date >= (current_date - make_interval(months => GREATEST(p_meses, 1)))
...
GRANT EXECUTE ON FUNCTION public.analise_mercado_acervo(text, int) TO authenticated;
