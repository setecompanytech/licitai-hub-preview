CREATE OR REPLACE FUNCTION public.vincular_lancamento_a_pedido(
  p_contrato_id uuid, p_contrato_item_id uuid, p_origem_aditivo_id uuid,
  p_numero_pedido text, p_descricao text, p_quantidade numeric,
  p_valor_unitario numeric, p_valor_total numeric, p_data_pedido date,
  p_tipo financeiro_tipo_lancamento, p_natureza financeiro_natureza,
  p_status financeiro_status_lancamento, p_data_competencia date,
  p_data_vencimento date, p_data_emissao date,
  p_tipo_documento financeiro_tipo_documento, p_numero_documento text,
  p_chave_acesso_nfe text, p_pessoa_id uuid, p_observacoes text
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Fallback: empresa criada pelo usuário OU em que ele é membro
  ...
  IF v_empresa_id IS NULL THEN
    SELECT em.empresa_id INTO v_empresa_id
