-- Adiciona colunas de fonte orçamentária e margem de preferência ao cache PNCP
ALTER TABLE public.pncp_editais_cache
  ADD COLUMN IF NOT EXISTS fonte_orcamentaria TEXT,
  ADD COLUMN IF NOT EXISTS margem_preferencia TEXT;

CREATE INDEX IF NOT EXISTS idx_pncp_cache_fonte_orc ON public.pncp_editais_cache (fonte_orcamentaria) WHERE fonte_orcamentaria IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_pncp_cache_margem ON public.pncp_editais_cache (margem_preferencia) WHERE margem_preferencia IS NOT NULL;

-- Atualiza a RPC de busca para suportar filtro por fonte e margem
