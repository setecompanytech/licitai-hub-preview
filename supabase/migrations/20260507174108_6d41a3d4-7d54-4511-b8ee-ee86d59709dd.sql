CREATE OR REPLACE FUNCTION public.restaurar_lancamentos_audit(p_limite integer DEFAULT 5000)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  WITH alvo AS (
    SELECT DISTINCT ON ((al.dados_antes->>'id')::uuid) al.dados_antes
    ...
    ORDER BY (al.dados_antes->>'id')::uuid, al.created_at DESC
    LIMIT p_limite
  ),
  rec AS (
    SELECT (jsonb_populate_record(NULL::public.financeiro_lancamentos, dados_antes)).* FROM alvo
  ),
  rec_filtrado AS (
    SELECT r.* FROM rec r
    WHERE NOT EXISTS (
      SELECT 1 FROM public.financeiro_lancamentos l
      WHERE l.empresa_id = r.empresa_id
        AND l.origem IS NOT DISTINCT FROM r.origem
        AND l.origem_ref IS NOT DISTINCT FROM r.origem_ref
        AND r.origem IS NOT NULL AND r.origem_ref IS NOT NULL
    )
  ),
  rec_dedup AS (
    SELECT DISTINCT ON (empresa_id, origem, origem_ref) *
    FROM rec_filtrado
    ORDER BY empresa_id, origem, origem_ref, updated_at DESC NULLS LAST
  )
...
SELECT public.restaurar_lancamentos_audit(5000) AS restaurados_lote_1;