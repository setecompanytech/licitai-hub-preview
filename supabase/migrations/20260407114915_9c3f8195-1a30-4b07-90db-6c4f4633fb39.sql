...
BEGIN
  -- Map modalidade_id to text pattern for fallback when modalidade_id column is NULL
  ...
  WITH resultados AS (
    ...
    WHERE
      ...
      -- Municipality: try IBGE code first, fallback to name match
      ...
      -- Modalidade: try numeric ID first, fallback to text pattern
