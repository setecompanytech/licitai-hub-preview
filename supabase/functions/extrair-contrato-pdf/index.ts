...
type DadosContrato = {
  ...
  assinatura_orgao?: string | null;
  assinatura_contratada?: string | null;
...
function normalizeContrato(data: DadosContrato) {
  ...
  return {
    ...
    // Só as duas formas que existem passam: valor inventado pela IA viraria
    // política de alerta sem ninguém ter declarado nada.
    forma_fornecimento: data.forma_fornecimento === 'unico' || data.forma_fornecimento === 'continuo'
      ? data.forma_fornecimento : null,
    ...
    assinatura_orgao: cleanString(data.assinatura_orgao),
    assinatura_contratada: cleanString(data.assinatura_contratada),
    assinatura_observacao: cleanString(data.assinatura_observacao),

    // Cláusula de reajuste (art. 92, V e §3º): índice, data-base e a frase
    // literal. É o insumo do alerta de aniversário anual — sem data-base o
    // alerta pede a data em vez de calcular do chute.
    indice_reajuste: cleanString(data.indice_reajuste)?.toUpperCase().slice(0, 30) ?? null,
    data_base_reajuste: normalizeDate(data.data_base_reajuste),
    reajuste_clausula: cleanString(data.reajuste_clausula),
            ...
            function: {
              ...
              parameters: {
                ...
                properties: {
                  ...
                  // prazo nenhum, inclusive o de divulgação do art. 94.
                  // A IA LISTA quem assinou de cada lado; QUEM CLASSIFICA é o
                  // código. Pedir a classificação pronta fez a IA devolver
                  // `so_orgao` para um documento cuja única assinatura era do
                  // "Contratado" — errado, e com cara de resposta.
                  assinatura_orgao: {
                    type: "string",
                    description: "Nome e cargo de quem assinou PELO ÓRGÃO CONTRATANTE (a Administração: secretário, comandante, ordenador de despesa, procurador do órgão). Null se não houver assinatura desse lado.",
                  },
                  assinatura_contratada: {
                    type: "string",
                    description: "Nome e cargo de quem assinou PELA EMPRESA CONTRATADA (sócio, representante legal, procurador da empresa; costuma aparecer rotulado como 'Contratado', 'Contratada' ou 'Fornecedor'). Null se não houver assinatura desse lado.",
                  },
                  assinatura_observacao: { type: "string", description: "O bloco de assinaturas como o documento mostra, literalmente." },
                  indice_reajuste: { type: "string", description: "Sigla do índice da cláusula de reajuste (IPCA, IGP-M, INPC, INCC, IPCA-E…). Só a sigla." },
                  data_base_reajuste: { type: "string", description: "Data-base do reajuste conforme a cláusula: a data da apresentação da proposta ou do orçamento estimado, de onde conta o interregno de 1 ano." },
                  reajuste_clausula: { type: "string", description: "A frase LITERAL da cláusula de reajuste, para conferência." },
                  modalidade: { type: "string" },
                  forma_fornecimento: {
                    type: "string",
                    enum: ["unico", "continuo"],
                    description: "Forma de fornecimento declarada na cláusula de entrega/execução: 'unico' quando o documento diz entrega única, integral, imediata ou em parcela única; 'continuo' quando diz parcelada, sob demanda, conforme requisição/ordem de fornecimento, ou fornecimento contínuo. Null se o documento não disser explicitamente.",
                  },
                  uf: { type: "string" },
      ...
      aviso: [
        escolha?.finish_reason === "length"
          ...
          : textoFoiRecortado
            ...
            : null,
        // Sem itens, a soma-contra-o-total não tem como vigiar o valor global:
        // ele fica SEM conferência nenhuma, e num processo com atas de vários
        // fornecedores é fácil o modelo pegar o total do processo. Dizer isso
        // é a diferença entre um rascunho conferível e um número com cara de lido.
        (!extracted.itens?.length && extracted.valor_global)
          ? "Nenhum item foi lido: o Valor Global veio SEM conferência — compare com o VALOR TOTAL do quadro OBJETO desta ata antes de salvar."
          : null,
