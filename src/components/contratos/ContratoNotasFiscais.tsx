...
import {
  FileText, FileCheck, CheckCircle2, XCircle, Clock, Send,
...
const statusCfg: Record<string, { label: string; variant: StatusVariant; icon: typeof CheckCircle2 }> = {
  ...
  rejeitada: { label: 'Rejeitada', variant: 'danger', icon: XCircle },
  cancelada: { label: 'Cancelada', variant: 'danger', icon: XCircle },
  inutilizada: { label: 'Inutilizada', variant: 'muted', icon: XCircle },
  registrada: { label: 'Registrada', variant: 'info', icon: FileCheck },
};
    ...
    <div className="space-y-4">
      ...
      <Card className="p-3 border-border bg-muted/40">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Button size="sm" variant="outline" className="shrink-0" onClick={() => navigate('/financeiro')}>
            <ExternalLink className="w-4 h-4" /> Ir ao Financeiro
          </Button>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="p-3">
          ...
          <p className="text-2xl font-bold tabular-nums">{notas.length}</p>
        ...
        <Card className="p-3">
          ...
          <p className="text-2xl font-bold tabular-nums text-success-ink">{fmt(totalAutorizado)}</p>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-1 text-muted-foreground text-xs mb-1"><Clock className="w-3 h-3" /> Pendentes</div>
          <p className="text-2xl font-bold tabular-nums text-warning-ink">{fmt(totalPendente)}</p>
        </Card>
        <Card className="p-3">
          ...
          <p className="text-2xl font-bold tabular-nums text-foreground">{fmt(totalEntrada)}</p>
        </Card>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button size="sm" variant="ghost" onClick={loadNotas}>
      ...
      {/* NF Table */}
      {loading ? (
        <div className="space-y-2" aria-busy="true" aria-label="Carregando notas fiscais">
        ...
        <Card className="p-8 text-center">
          ...
          <p className="text-lg font-semibold">Nenhuma nota fiscal vinculada a este contrato</p>
          <p className="text-sm text-muted-foreground mt-1 max-w-md mx-auto">Para emitir uma NF vinculada a este contrato, acesse <strong>Financeiro → NF Saída</strong> e selecione este contrato no campo "Vincular ao Contrato".</p>
          <Button variant="outline" className="mt-4" onClick={() => navigate('/financeiro')}>
            <ExternalLink className="w-4 h-4" /> Ir ao Financeiro
          </Button>
        </Card>
      ) : (
