      ...
      <Card>
        <CardContent className="p-0 overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={6} className="p-2"><Skeleton className="h-8 w-full" /></td></tr>
              ) : contas.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-12 text-muted-foreground">Nenhuma conta cadastrada.</td></tr>
              ) : (
                contas.map((c) => (
                  <tr key={c.id} className="border-t hover:bg-muted/30">
                    <td className="px-3 py-2 font-medium">{c.nome}</td>
                    <td className="px-3 py-2 text-muted-foreground">{TIPOS.find((t) => t.value === c.tipo)?.label ?? c.tipo}</td>
                    <td className="px-3 py-2 text-muted-foreground">{c.banco_nome ?? "—"}</td>
                    <td className="px-3 py-2 text-muted-foreground tabular-nums">{c.agencia || c.conta ? `${c.agencia ?? "—"} / ${c.conta ?? "—"}` : "—"}</td>
                    <td className="px-3 py-2 text-right tabular-nums">{formatBRL(Number(c.saldo_atual ?? 0))}</td>
