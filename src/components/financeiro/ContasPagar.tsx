...
import {
  Plus, Trash2, Loader2, ArrowDownCircle, DollarSign, AlertTriangle,
...
const categorias = [
  ...
  'Frete/Logística', 'Serviços', 'Outros',
];
...
export default function ContasPagar() {
  ...
  const handleSave = async () => {
    ...
    setDialogOpen(false);
    setForm({ descricao: '', fornecedor: '', valor: '', data_vencimento: '', categoria: '', nota_fiscal: '', observacoes: '' });
    load();
  };
  ...
  const totalPago = contas.filter(c => c.status === 'pago').reduce((s, c) => s + c.valor_pago, 0);
                  ...
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        {c.status !== 'pago' && (
                          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => handlePagar(c.id)} title="Marcar como pago">
