...
export default function PlanilhaPrecos({ itens, setItens }: PlanilhaPrecosProps) {
  ...
  const addItem = () => {
    setItens([...itens, { item: String(itens.length + 1), descricao: '', quantidade: '', unidade: 'UN', valorUnitario: '', valorUnitarioExtenso: '', valorTotal: '', valorTotalExtenso: '' }]);
  };
  ...
  const handleDownloadTemplate = () => {
    const headers = ['Item', 'Descrição', 'Quantidade', 'Unidade', 'Valor Unitário (R$)', 'Valor Total (R$)'];
    const sampleRows = [
      ['1', 'Exemplo de produto/serviço', '10', 'UN', '150.00', '1500.00'],
      ['2', '', '', 'UN', '', ''],
    ];
    const ws = XLSX.utils.aoa_to_sheet([headers, ...sampleRows]);
    ws['!cols'] = [{ wch: 6 }, { wch: 40 }, { wch: 12 }, { wch: 10 }, { wch: 18 }, { wch: 18 }];
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const parsed: EditalItem[] = dataRows.map((r, i) => {
        ...
        const unit = parseFloat(String(r[4] || '0').replace(',', '.')) || 0;
        const total = parseFloat(String(r[5] || '0').replace(',', '.')) || (qty * unit);
        ...
        <table className="w-full text-sm border border-border rounded-lg">
          <thead>
            <tr className="bg-muted/50">
              ...
              <th className="px-2 py-2 text-left w-40">Vlr Unit. Extenso</th>
              <th className="px-2 py-2 text-left w-24">Vlr Total (R$)</th>
              <th className="px-2 py-2 text-left w-40">Vlr Total Extenso</th>
          ...
          <tbody>
            {itens.map((item, i) => (
              <tr key={i} className="border-t border-border/50">
                ...
                <td className="px-2 py-1">
                  <span className="text-xs text-muted-foreground italic leading-tight block truncate max-w-[160px]" title={item.valorUnitarioExtenso}>
                ...
                <td className="px-2 py-1">
                  <span className="text-xs text-muted-foreground italic leading-tight block truncate max-w-[160px]" title={item.valorTotalExtenso}>
