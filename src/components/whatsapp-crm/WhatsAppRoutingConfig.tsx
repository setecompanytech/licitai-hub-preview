...
import {
  Bot, Loader2, Save, Copy, Settings2, Clock, Zap,
  Route, Shield, CheckCircle2, ArrowRight, Globe
} from 'lucide-react';
...
export default function WhatsAppRoutingConfig() {
  ...
  const loadConfig = async () => {
    ...
    if (data) {
      setConfig({
        id: data.id,
        ativo: data.ativo,
        resposta_automatica: data.resposta_automatica,
  ...
  const handleSave = async () => {
    ...
    const payload: any = {
      user_id: user!.id,
      ativo: config.ativo,
      resposta_automatica: config.resposta_automatica,
      ...
      provider_url: config.provider_url || null,
      provider_instance: config.provider_instance || null,
    };
  ...
  const handleTestWebhook = async () => {
    try {
      ...
      toast.success(`Teste executado! Setor: ${res.data?.setor} (confiança: ${(res.data?.confianca * 100).toFixed(0)}%)`);
      loadLogs();
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5">
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Copie esta URL e cole nas configurações de webhook do seu provedor de WhatsApp (Evolution API, Z-API ou Twilio).
        </p>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Cada mensagem recebida é analisada pela IA que identifica o setor
                    (Licitações, Jurídico, Financeiro ou Documentos) e encaminha automaticamente.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                checked={config.resposta_automatica}
                onCheckedChange={v => setConfig(p => ({ ...p, resposta_automatica: v }))}
              />
            </div>
            ...
            {config.resposta_automatica && (
              <>
                <div>
                  <Label className="text-xs">Mensagem de boas-vindas (horário comercial)</Label>
                  <Textarea
                    value={config.mensagem_boas_vindas}
                    onChange={e => setConfig(p => ({ ...p, mensagem_boas_vindas: e.target.value }))}
                    className="mt-1 text-xs"
                    rows={3}
                  />
                  <p className="text-[10px] text-muted-foreground mt-1">
                    💡 Dentro do horário, a IA gera respostas personalizadas. Esta mensagem é o fallback.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={e => setConfig(p => ({ ...p, mensagem_fora_horario: e.target.value }))}
                    className="mt-1 text-xs"
                    rows={3}
                  />
                </div>
        ...
        <Card className="p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold">Horário de Atendimento</h2>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="pt-2 border-t">
              ...
              <Select
                ...
                onValueChange={v => setConfig(p => ({ ...p, provider: v }))}
              >
              ...
              <div className="mt-3 space-y-2">
                <div>
                  <Label className="text-xs">URL Base do Provedor</Label>
                  <Input
                    placeholder={
                      config.provider === 'evolution' ? 'https://sua-api.com'
                        : config.provider === 'zapi' ? 'https://api.z-api.io/instances/...'
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={config.provider_instance}
                    onChange={e => setConfig(p => ({ ...p, provider_instance: e.target.value }))}
                    className="mt-1 text-xs"
                  />
                </div>
              </div>
              ...
              <div className="mt-3 p-2 rounded bg-amber-500/5 border border-amber-500/10">
                <p className="text-[10px] text-amber-600">
                  ⚠️ A chave de API do provedor deve ser configurada separadamente por segurança.
            ...
            <div className="space-y-2">
              {logs.map(log => (
                <div key={log.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-muted/30 text-xs">
                  ...
                  <Badge className={`text-[10px] ${setorColor[log.setor_destino] || 'bg-muted'}`}>
                    {log.setor_destino}
                  </Badge>
