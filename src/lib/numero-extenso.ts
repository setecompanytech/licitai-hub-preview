...
export function valorPorExtenso(valor: number): string {
  ...
  for (let i = grupos.length - 1; i >= 0; i--) {
    ...
    if (i === 0) {
      ...
      partes.push(`${texto} ${grupos[i] === 1 ? escala.singular : escala.plural}`);
    }
  }

  let resultado = '';
  if (partes.length > 1) {
    resultado = partes.slice(0, -1).join(', ') + ' e ' + partes[partes.length - 1];
  ...
  if (centavos > 0) {
    ...
    resultado += `${centavosTexto} centavo${centavos === 1 ? '' : 's'}`;
  }
