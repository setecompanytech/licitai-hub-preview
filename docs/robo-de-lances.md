...
- [~] **Envio da proposta ao robô consertado** — 16/09/2026 (`robo-lances-webhook` **v52** e `enviar-proposta-portal` **v17**, 18:38; versão `2026-09-16.11`). Pergunta do Ian: "qual é a limitação? dá pra fazer logo?". Ao levantar o caminho, ele estava **quebrado antes da tela do portal**: a função `enviar-proposta-portal` (botão "Enviar proposta via Agente Cloud" da tela de Proposta) lia colunas que não existem em `credenciais_portais` (`usuario_cifrado`, `senha_cifrada`, `portal`, `empresa_id`) — respondia "credenciais não cadastradas" para todo mundo —, não mandava login, senha nem UASG ao robô e pegava o agente com `.single()` (falha com duas configurações). Agora o envio é a ação `enviar-proposta` do webhook, com as mesmas peças do envio de sessão (membro da empresa, robô da empresa ligado, agente mais recente, credencial em claro só para o robô, chave certa); a função antiga só encaminha. A tela pede a **UASG** no Compras.gov e, quando o robô responde que ainda não cadastra proposta no portal, mostra um aviso amarelo "Cadastro automático ainda não disponível neste portal", com o motivo — não "falha no envio"
  - **a limitação que continua**: o preenchimento do formulário de proposta do Compras.gov (onde fica "incluir proposta", nomes dos campos de valor, marca, fabricante, modelo e descrição, anexos e declarações) **não foi escrito porque a tela nunca foi vista**. Escrever no escuro repetiria o `enviarLance` de fevereiro — seletores de chute —, e proposta é compromisso legal da empresa. Com uma compra escolhida pelo Rafael e pela Izabelle, uma passada do robô com o gravador (e a tela remota do admin) grava a tela, e o `enviarProposta` do Compras.gov é escrito a partir da gravação, parando antes do envio final na primeira vez
  - o robô, ao enviar, ainda abre um Chrome **sem a sessão guardada** (rota `/api/proposta/enviar`): ajustar para o perfil persistente junto com o `enviarProposta`, senão o captcha volta
- [ ] Cadastro da proposta no Compras.gov pelo robô: valor, marca, fabricante, modelo e descrição por item; declarações do portal só quando o cadastro disser — **depende de ver a tela numa compra escolhida por eles**
- [x] **PAUSADO em 17/09/2026, por decisão do Ian — o foco passou a ser a disputa.** Motivos, conferidos: o Rafael não pediu essa automação (a queixa dele foi o robô ser uma ilha, e o "do cadastramento à homologação" foi atendido pelo vínculo com o processo e o Kanban); a esteira com proposta veio do Giovanny (D6, WhatsApp 15/09 13:03), a confirmar com ele; o próprio ConLicitação, visto na reunião, não cobre o Compras.gov no robô web; e a proposta tem dias de prazo para a equipe montar e conferir no portal. O que foi feito:
  - **o cadastro no robô foi escrito e depois REMOVIDO do código no mesmo dia** (a pedido do Ian: pausado, sem teste de tela e nunca visto funcionando, seria ruído no módulo do Compras.gov, que é o mesmo arquivo da disputa). Fica no histórico do git — `git show e643d1e8` —, desfeito pelo commit seguinte. Era `ComprasGovPortal.enviarProposta`, escrito a partir do passo a passo oficial (PDF do Senado, PE 53/2023, "Cadastramento de proposta no Compras.gov.br") e do FAQ do pregão eletrônico (gov.br/compras): área do fornecedor → "Dispensa/Licitação Eletrônica" → "Compras eletrônicas" → "Todas as compras" → pesquisa → "Acompanhar compra" → "Cadastrar propostas". Três níveis — reconhecer (só lê), ensaio (preenche e confere, sem salvar; o Termo de Aceitação só com a conferência da empresa) e salvar — e recusa em proposta por grupo (FAQ 5.1.1), item em percentual (FAQ 4.1), item já cadastrado (só se troca excluindo, FAQ 1.25) e campo não achado pelo rótulo. **Sem teste de tela ainda**, e o primeiro clique (o menu da área do fornecedor) nunca foi visto: o gravado em 10/09 difere do manual de 2023;
  - as duas travas que seguravam esse código (`PORTAIS_COM_PROPOSTA_EM_TESTE` e `PORTAIS_COM_PROPOSTA_LIBERADA`) saíram junto: sem `enviarProposta` no portal, a rota `/api/proposta/enviar` já responde 501 "não implementado" antes de abrir o navegador, como antes;
  - **a tela antiga "Envio de Proposta" não manda mais para o Compras.gov**: ela numerava os itens pela ordem do carrinho (1, 2, 3), e no Compras.gov o item tem o número da compra — o valor do item 12 iria para o item 2. Agora avisa que a proposta é feita no portal pela equipe (versão `2026-09-17.4`);
  - **o desenho combinado para quando voltar**: sair da página da disputa (itens com o número da compra), com uma conferência manual antes do robô — itens, declarações desmarcadas por padrão, "revisei" obrigatório e registro de quem conferiu —, e o salvar como segundo clique. Regra do Ian: mesmo o "reconhecer" só numa compra escolhida pelo Rafael e pela Izabelle
...
- **16/09 — o teste da sessão guardada, em rodadas** (disputa do 7/2026 reagendada para 2 minutos à frente, despachada pelo agendador sem clique):
  ...
  **Dois achados de caminho**:
  - **proposta desclassificada**: na tela do item 5 (vista pelo Ian na tela remota às 13:15 e gravada), 6 das 13 propostas estavam "Desclassificada", três no topo — o leitor tomava a primeira como melhor lance (R$ 2.000,00, contra R$ 2.785,00 da melhor válida). Corrigido e testado com o texto real (commit local, a instalar — instalado na VPS e enviado aos remotos no mesmo dia): desclassificada não conta como melhor lance nem como posição, e a proposta da própria empresa desclassificada faz o robô aguardar;
...
- [~] Medir: depois de alguns pregões, contar os desfechos em `logs/logins.jsonl` e registrar aqui quantos logins o perfil guardado evitou e **por quanto tempo** o gov.br mantém a sessão
  - **parcial, 16/09 às 17:44**: último clique no certificado às **13:31**; depois dele, só `sessao-reaproveitada` (13:46, 14:55, 16:45 ×2, em 2–3 s) e o vigia `logado` a cada 20 minutos até **17:44** — a sessão do gov.br valeu **pelo menos 4h13** sem clique novo, com o vigia passando por ela. Falta ver se atravessa a noite (a primeira disputa da manhã seguinte responde); para o caso de não atravessar, a entrada antecipada abaixo
  - **primeira medição completa, 17/09** (`logs/logins.jsonl`, horário de Brasília): clique no certificado em 16/09 às **13:31:43** (`certificado-com-clique`, o último registrado); `sessao-reaproveitada` até 17/09 às 02:54 (testes do Ian); vigia `logado` até **05:58:32** e **`vigia-vencida` às 06:18:31**. **A sessão durou entre 16h27 e 16h47 depois do clique.** Aponta para **prazo fixo contado do login**, não para inatividade: de 16:45 a 02:48 o robô não entrou em nada e a sessão atravessou essas 10 horas, e ela venceu com o vigia passando a cada 20 minutos. É uma medição só — a próxima vencida confirma. Consequência prática: **um clique pela manhã cobre o dia** (login às 08:00 valeria até por volta de 00:30), e o caminho da tabela "por tempo fixo" (aquecimento diário) é o que vale; a entrada antecipada de 60 minutos já cobre o primeiro pregão do dia. O vigia, depois da vencida, para de conferir até uma disputa logar de novo (`vencida-sem-novo-login`)
  - uma conferência às **17:21 deu `vigia-indefinido`** (39 s): em 20 s a página não chegou nem à tela de login do gov.br nem à área logada. A seguinte, às 17:26, deu `logado` em 3 s — foi passageiro. O defeito é o vigia **não registrar em que tela parou**
- [x] **Vigia: registrar a tela quando der "indefinido"** (pedido do Ian em 16/09: "não esqueça") — escrito e no ar em 16/09: a linha `vigia-indefinido` em `logs/logins.jsonl` ganha `detalhe` com o endereço **sem a parte de parâmetros** (pode carregar token), o título da página e "com captcha" quando houver hCaptcha na tela — "indefinido" repetido vira diagnóstico, não palpite. 2 testes (um com a página já fechada). `vigia-sessao.js` `02d8450e…` na VPS às 17:53, md5 igual ao template, conferido antes que o instalado era o das 14:06 (`2c80e779…`) e que a única diferença era esta; backup `.bak-20260916-1753`, 0 sessões, nenhum Chrome órfão depois do `pm2 restart`, `/health` online, trava `[]`, vigia ligado
- [x] **Entrada antecipada quando a sessão do gov.br venceu** (o "login antecipado" desta fase) — escrito e no ar em 16/09 (`robo-lances-webhook` **v49**, 17:49). Com a sessão vencida, o captcha aparecia na entrada de 15 minutos antes, e o pedido espera 10: sem ninguém olhando naquele quarto de hora, o pregão ia sem robô. Agora:
  - o agendador lê a sessão do gov.br da conta pelo vigia (mesma leitura do lembrete, que virou uma função só): **vencida, despacha 60 minutos antes**; logada ou sem conferência, 15, como antes (entrar cedo à toa ocupa vaga do servidor). O despacho antecipado fica no `webhook_log` como `entrada-antecipada`;
  - **captcha que expira sem clique não custa mais o pregão**: a entrada do agendador que falha com "O login do gov.br não foi concluído", ainda entrando (nunca chegou à sala) e com o pregão ao alcance (até 20 minutos depois do início), volta à agenda e entra de novo no minuto seguinte — cada entrada chama a equipe para o clique —, até 5 vezes (`tentativas_envio`, o mesmo limite das falhas passageiras). O dono recebe "🔁 Robô não entrou, tentando de novo" na primeira, e "Robô não entrou" se acabar; a linha do tempo da disputa registra cada uma. Pelo botão (sem `enviada_em`) continua sem nova tentativa: quem enviou decide;
  - lembrete e aviso de sessão vencida dizem a hora certa ("o robô entra às 08:00, 1 hora antes, para dar tempo ao clique"). A tela da disputa segue dizendo "entra 15 minutos antes": ela não conhece o vigia;
  - 6 testes novos em `prontidao.test.ts`. Provado no ar só que o agendador segue rodando até o fim com a v49 (chamada ao `/health` da VPS às 17:50:01, vinda do Supabase); **a entrada antecipada em si se prova quando a sessão vencer** — hoje o vigia a vê logada
...
- [x] **Sessão encerrada no meio do login** não vira mais erro — escrito e no ar em 16/09. Antes, parar a sessão enquanto o login esperava o captcha fazia o login falhar (a aba fechou) e mandava o aviso vermelho "Robô parou", com status "erro" por cima de "encerrado" (visto na 2ª rodada, 13:28). Agora a parada pedida durante a entrada só fica no log ("Sessão encerrada durante a entrada — sem aviso de erro"). Junto, duas correções achadas no mesmo caminho:
  - **o robô responde sem esperar o captcha**: a rota `/sessao/iniciar` só respondia depois do login e da navegação. Com o gov.br pedindo o clique (até 10 minutos), quem chamou estourava o tempo, marcava a sessão como erro, e o agendador podia despachar a mesma disputa de novo com a primeira ainda esperando. Agora, se a entrada não termina em 40 s (`SEGUNDOS_RESPOSTA_INICIAR`), o agente responde `202 entrando` e segue; o resultado chega pelos avisos de sempre (entrou na sala, pedido de captcha, erro). O webhook trata o 202 como "enviando" — no botão, a mensagem "Sessão aceita. O robô ainda está entrando no portal — você será avisado quando ele chegar à sala"; no agendador, a disputa fica despachada e não é repetida;
  - **disputa sem teto de lances virava disputa de 20 lances**: a rota trocava "máximo de lances" vazio por 20 sem ninguém ver. Vazio agora é sem teto — vai até o piso, como decidido para a estratégia
  - **no ar**: `robo-lances-webhook` **v48** às 17:18; na VPS às 17:24, `index.js` `8cceaffe…` e `session-manager.js` `8f4f9645…`, md5 igual ao template, backups `.bak-20260916-1723`, 0 sessões antes do `pm2 restart`; `/health` online, 14 rotas, limite 6, trava `[]`, vigia ligado. Os outros quatro arquivos do agente (`estrategia.js`, `browser.js`, `portals/comprasgov.js`, `portals/base-portal.js`) já eram iguais ao template. 1 teste novo (parada durante o login não manda erro)
...
- [x] **A tela de acordo com as regras novas do robô** (pedido do Ian, 16/09) — conferido e corrigido (versão `2026-09-16.3`, enviada aos remotos em 16/09; o Publish no Lovable segue pendente):
...
- [~] **Acompanhamento sem tela remota (D13)** — ordem definida em 16/09: vem antes de "uma aba por pregão". Escrito, no servidor e provado no banco; falta só a tela publicada (itens abaixo)
  ...
  - [ ] pôr no ar — servidor feito em 16/09: `robo-lances-webhook` **v41** às 14:38; na VPS, `session-manager.js` (md5 `af8444ea…`) e `portals/comprasgov.js` (`1e715087…`) iguais ao template, backups `.bak-20260916-1439`, 0 sessões antes do `pm2 restart` das 14:39, `/health` online, 14 rotas, trava `[]`, vigia ligado. Push feito em 16/09 às 15:10 nos dois remotos (`5ba8d0ae..7fb4bbd7`), com tipos, lint e 165 testes do robô verdes. **Falta** o Publish da versão `2026-09-16.4` no Lovable (o domínio servia a `2026-09-16.2` às 15:10). **Às 17:58 o domínio ainda serve a `2026-09-16.2`**, e o repositório está na `2026-09-16.9`: os commits depois de `9a47c5dd` ficaram **locais, sem push, a pedido do Ian** ("ainda não dê push, temos que seguir adiante no desenvolvimento") — as telas das versões `.5` a `.9` (data do processo, aviso no canto e sininho, calendário, quadro por item, busca no Compras.gov) só chegam ao ar depois do push e do Publish. **Push feito às 19:11 de 16/09** (`366ed5e0` em `origin` e `sete`, remoto conferido parado antes); falta o Publish
  ...
  - [ ] prova com uma disputa da BAQPLAST no 7/2026: 8º lugar no item 1, R$ 4.999,70, visível no quadro e nas colunas — **provada no banco em 16/09**; falta ver na tela depois da publicação da `2026-09-16.4`. **Não será com a BAQPLAST**: a disputa de prova foi apagada no incidente das duas empresas (abaixo) e teste não mexe mais em vínculo de empresa — a prova na tela fica para uma disputa da Santa Rosa
  ...
  - [x] **o agendador confere se o dono da disputa ainda é membro da empresa** antes de despachar — escrito e no ar em 16/09 (`robo-lances-webhook` **v47**): membro da empresa ou administrador da plataforma, a mesma regra do botão; "fora" confirmado reserva a disputa (não tenta de novo a cada minuto) e avisa o dono "Robô não entrou — quem cadastrou esta disputa não é mais membro da empresa…"; leitura que falha não barra. O lembrete de prontidão mostra isso como pendência grave. Original: O envio pelo botão recusa quem não é membro; o agendador não confere. Então quem saiu da empresa, ou uma disputa gravada direto no banco, continua mandando o robô entrar com a credencial dessa pessoa. Achado em 16/09, ao montar a prova da BAQPLAST
...
- [x] **Aviso do robô também no canto da tela** (pedido do Ian em 16/09: "além do sininho, um toast — não é para substituir") — escrito em 16/09, versão `2026-09-16.6`. `LembreteDoRobo`, no mesmo desenho e na mesma pilha dos lembretes de certidão e de convocação: título, "Robô de Lances · há 5 min", a mensagem e o botão para onde leva ("Abrir a disputa", "Abrir a tela remota do robô"). Faixa vermelha para urgente/alerta, amarela para lembrete, verde para informativo. Fica até ser dispensada, e aparece também para quem abre o sistema depois (avisos do robô não lidos das últimas 24 horas, até 3 visíveis). Dispensar **não** marca como lida: o sininho continua sendo o registro. O toast simples que o `AppLayout` mostrava para toda notificação deixa de sair para as do robô, que viraria o mesmo aviso duas vezes. Regra em `src/lib/robo/avisos-do-robo.ts` (8 testes)
  - **17/09 — a caixinha passou a sumir sozinha** (pedido do Ian: "aparece e depois desaparece", para não poluir junto dos lembretes de certidão e regularidade, que continuam fixos). Fica **8 s** o aviso comum e **15 s** o urgente; o mouse em cima (ou o foco do teclado) segura enquanto a pessoa lê, e ao sair conta só o que faltava. Ao abrir o sistema, só os **3 mais recentes** aparecem; os mais antigos já contam como vistos. Sumir não marca como lida: tudo continua no sininho. Versão `2026-09-17.1`, 4 testes de tela (`LembreteDoRobo.test.tsx`) e 2 de regra
...
- [ ] **WhatsApp** para os mesmos avisos — **bloqueado**: `whatsapp-envio` hoje só simula o envio (grava "simulado", não manda nada); precisa de provedor contratado (Z-API, Evolution ou Twilio), decisão do Ian e do Rafael
- [x] **Kanban automático — a parte que o robô consegue afirmar** — escrito e no ar em 16/09 (`robo-lances-webhook` **v51**, 18:34), pedido do Ian ("e o kanban, por que ainda não foi feito?"). Regra em `_shared/robo-kanban.ts` (6 testes):
  - **a proposta da empresa aparece na sala** (leitura do robô, `tem_proposta`) → o processo vai para **Em Disputa**, com mensagem no mural ("🤖 Processo movido para Em Disputa — o robô viu a proposta da empresa na sala da compra 07/2026 (Compras.gov.br). Estava em Proposta Enviada."). Só a partir de Monitorando, Em Análise ou Proposta Enviada; **nunca anda para trás** e nunca mexe em Vencida, Homologada, Perdida ou Arquivada. A troca é condicional no banco (só se o status ainda for um desses), e falha só registra no `webhook_log`, sem atrapalhar a leitura da sala;
  - **no fim da sessão**, o aviso "🏁 Robô encerrou" e o mural trazem a **última posição de cada item** ("item 1: 8º lugar (nosso R$ 4.999,70, melhor R$ 3.100,00); item 5: proposta desclassificada");
  - **o que continua com gente, e por quê**: Vencida e Perdida. Arrematar na sala não é vencer (vem aceitação e habilitação), e Perdida exige o motivo em `comercial_perdas` — o banco recusa sem ele. Mover para esses dois depende de ler o resultado final da compra, o que fica para depois do mapeamento da sala
  - **ainda não provado ao vivo**: acontece na próxima disputa em que a empresa tenha proposta e a disputa esteja ligada a um processo
- [x] **O resultado da compra volta ao processo** — a outra ponta do "do cadastramento à homologação" (Rafael). Escrito e no ar em 16/09 (`robo-lances-webhook` **v53** e `compra-comprasgov` **v3**, 18:47). O robô não sabe quem venceu olhando a sala, mas **os dados abertos do Compras.gov publicam, por item, a situação e o fornecedor vencedor com o CNPJ** (conferido em compras homologadas de junho: "Homologado" com `codFornecedor`, nome, valor e data; "Deserto" sem fornecedor). De hora em hora (minuto 23, em segundo plano, sem atrasar o agendador), para as disputas do Compras.gov ligadas a processo em **Proposta Enviada, Em Disputa ou Vencida**, com a sessão já passada (até 120 dias), no máximo 15 compras por vez; com **todos os itens da disputa decididos**:
  - **a empresa venceu algum item** (o CNPJ dela é o homologado) → processo em **Homologada**, com mural, aviso e e-mail ("🏆 Compra homologada com vitória — … A empresa venceu o item 2 (R$ 21,00). Item 1: deserto. Processo movido para Homologada."). Homologação publicada já passou pela habilitação e pelos recursos — é o contrário de mover pelo fim da fase de lances, que continua não movendo;
  - **homologada só para outros fornecedores** → aviso com quem venceu cada item e por quanto, pedindo o motivo da perda; **não move para Perdida**, que exige o motivo em `comercial_perdas`;
  - **sem vencedor** (deserto, fracassado, cancelado) → aviso para registrar o desfecho;
  - item ainda sem resultado → nada; confere de novo na hora seguinte. Cada desfecho avisa uma vez (marca `resultado-comprasgov` no `webhook_log`)
  - regra em `_shared/robo-resultado.ts` (8 testes com duas compras reais: Emílio Ribas, com item homologado e desertos, e CRO/RR). A busca nos dados abertos virou uma só, usada pela tela e pelo agendador
  - **achado no caminho**: depois do resultado, os dados abertos trazem **o mesmo item em duas linhas** ("Em andamento" e "Homologado", mesmo `idCompraItem`). O cadastro da disputa mostraria o item duplicado numa compra já homologada — agora junta por número
  - tipos conferidos com `deno check` numa cópia fora do repositório: os mesmos 6 erros antigos do webhook antes e depois, nenhum novo
  - **ainda não provado ao vivo**: acontece quando uma compra de disputa ligada a processo tiver resultado publicado
- [x] **A UASG vem do processo** — escrito e no ar em 16/09 (`robo-lances-webhook` **v54**, 18:57; versão `2026-09-16.12`). O processo (`licitacoes`) não tem coluna de UASG, e a disputa **importada do Kanban nascia com o campo vazio** — sem ele o robô não acha a compra no Compras.gov (o número se repete entre órgãos) e o lembrete acusava "falta a UASG". Agora a UASG sai do **espelho do PNCP** (`pncp_editais_cache`), pelas coordenadas do processo (número de controle, ou CNPJ + ano + sequencial — princípio 4): o link do Compras.gov traz o id da compra (UASG = 6 primeiros dígitos de `compra=92531505000072026`), senão `uasg_codigo`, senão `codigo_unidade`:
  - **no cadastro**, ao importar o processo: preenche a UASG (sem sobrescrever o que a pessoa digitou; outro processo limpa a do anterior) e diz "UASG puxada do processo (espelho do PNCP). Confira no edital.";
  - **no agendador e no lembrete**, para disputa já cadastrada sem UASG e ligada a processo: usa a do espelho e a disputa passa a guardá-la;
  - 4 testes (3 no front, 1 da regra do servidor); `deno check` do webhook com os mesmos 6 erros antigos
- [x] **Habilitação no lembrete do pregão** — escrito e no ar em 16/09 (`robo-lances-webhook` **v55**, 19:01). Vínculo com a pasta de documentos: a habilitação é pedida logo depois dos lances, e o lembrete da véspera e o de 1 hora antes passam a citar os **documentos da empresa com validade vencida ou que vencem antes do dia da sessão** ("2 documentos da empresa não estarão válidos no dia da sessão, e a habilitação vem logo depois dos lances: CNDT (30/08), CND Federal (16/09)"), até 3 nomes. Aviso, não impedimento: o robô entra do mesmo jeito. Leitura dos documentos que falha só deixa de avisar. `documentosQueVencemAteASessao` em `_shared/robo-prontidao.ts` (2 testes; vale no dia da sessão não entra)

**Fase 7 — vários pregões ao mesmo tempo** (pergunta do Ian em 16/09; depois do acompanhamento e da Fase 8)
- [x] **Um navegador por empresa, uma aba por pregão**: a segunda disputa da mesma identidade abre aba nova no Chrome já logado, em vez de um Chrome com perfil temporário; o Chrome fecha quando a última disputa da empresa termina; o vigia convive com ele — escrito em 16/09 e instalado na VPS às 16:39/16:43 (enviado aos remotos no push de `366ed5e0`). O gerente de sessões guarda um Chrome por perfil (conta do gov.br): a segunda disputa da mesma conta abre uma aba nova nele, e o login ali só confirma a sessão (os cookies são do navegador). Encerrar uma disputa fecha só as abas dela; o Chrome fecha com a última, e a próxima disputa da conta abre outro. Contas diferentes seguem em Chromes separados. A aba de outra disputa **nunca é adotada** quando a aba desta morre (`adotarAbaViva` recebe as abas proibidas). "Trazer para a frente" na tela remota passa a trazer também a aba certa. O vigia não confere perfil com Chrome aberto. `UMA_ABA_POR_PREGAO=false` no `.env` volta ao comportamento antigo. 7 testes novos (`abas-por-pregao.test.ts`); 294 na área do robô
- [x] **Todos os itens do pregão**, e não só o primeiro: o laço acompanha cada item configurado, com o piso e a estratégia dele — escrito em 16/09, no ar no mesmo dia (abaixo) e enviado aos remotos no push de `366ed5e0`. A cada rodada o laço lê o chat uma vez e percorre os itens: cada um com piso, estratégia, margem, modo e intervalo mínimo (lidos por item na entrada), último melhor lance, nosso valor e lance fechado/desempate próprios; o valor inicial de cada item, sem leitura do portal, é o preço de venda dele. `decidirLance` passou a dizer o **escopo** do encerramento: o teto de lances é da disputa inteira e tira o robô da sala; piso alcançado, item encerrado no portal ou proposta não classificada acabam só com aquele item, e a sessão encerra quando todos acabam ("Todos os itens encerraram — item 1: …; item 2: …"). Um item que falha na leitura não para os outros. Os métodos de leitura do Compras.gov recebem o número do item; os callbacks de lance levam o item. O webhook guarda o último estado de cada item em `estado_sala.por_item` e compara cada item com o seu (sem isso a troca de item viraria "mudou de posição"); o quadro da sala mostra uma linha por item (versão `2026-09-16.8`). Testes: 4 novos no laço (vários itens, piso por item, teto somando itens, estado por item), 3 no estado da sala, 2 no quadro — 321 no total da área do robô
- [x] Testes simulando duas disputas da mesma empresa ao mesmo tempo, antes de instalar — `abas-por-pregao.test.ts` (duas disputas da mesma conta num Chrome só, encerrar uma sem derrubar a outra, freio de emergência, contas diferentes separadas, a aba de um pregão nunca adotada pelo outro) e `laco-de-lances.test.ts` (vários itens)
  - **no ar em 16/09** (push feito depois, em `366ed5e0`): `robo-lances-webhook` **v46** às 16:38 (estado por item); na VPS, `session-manager.js`, `estrategia.js`, `index.js`, `portals/comprasgov.js` e `portals/base-portal.js` instalados às 16:39:36 (backups `.bak-20260916-1639`, 0 sessões, vigia tinha conferido "logado" às 16:39:31), e o `session-manager.js` de novo às 16:43 (`06f973cd…`, backup `.bak-20260916-1643`) com a correção abaixo; `/health` online, trava `[]`, vigia ligado. `callback.js` da VPS difere do template só num comentário e ficou como estava
  - **corrida corrigida antes da prova**: duas disputas da mesma conta chegando juntas (dois "Enviar ao robô" quase ao mesmo tempo, ou a primeira ainda no captcha) faziam a segunda abrir outro Chrome com login do zero. Agora a conta fica marcada como "Chrome abrindo" desde a reserva do perfil até o fim do login, e a segunda espera e entra numa aba dele — o teste escrito para isso falhou antes da correção e passou depois
- [x] Prova ao vivo: dois pregões no mesmo horário, ou um pregão real com vários itens — **feita em 16/09, 16:45–16:48**, com as duas coisas ao mesmo tempo: a disputa de prova da BAQPLAST no 7/2026 com os itens 1, 2, 3 e 5, e a da Santa Rosa no mesmo 7/2026, as duas da mesma conta e marcadas para 2 minutos depois (sem lembrete por e-mail: faltavam menos de 15 minutos)
  - 16:45:01 a primeira abriu o Chrome **depois de esperar o vigia** terminar a conferência das 16:45; login "sessão reaproveitada" em 3 s; leu modo e intervalo de cada item (item 5: "Cota reservada ME/EPP do item 4"); ativa às 16:45:20
  - 16:45:20 a segunda: "Mesma conta de outra disputa aberta — nova aba no Chrome ja logado (2 pregoes nele)"; login em 3 s, sem certificado e sem captcha; ativa às 16:45:34 — **duas sessões num Chrome só**
  - a da BAQPLAST leu os 4 itens a cada rodada ("Item 1: 13 proposta(s)…", "Item 2: 12…", "Item 3: 11…", "Item 5: 13 proposta(s), 9 desclassificada(s)…"), três rodadas; a da Santa Rosa, o item dela, três rodadas; **0 erros de aba** (Session closed, Target closed, frame descolado)
  - 16:48:22 encerrada a da BAQPLAST: "Aba fechada; o Chrome segue com 1 outra(s) disputa(s) da mesma conta" e só a aba dela fechou; 16:48:26 encerrada a da Santa Rosa e o Chrome fechou; `/health` 0 sessões, trava `[]`
- [x] Observar se um segundo login do mesmo CPF derruba o primeiro — **não derrubou** na prova de 16/09: o segundo login foi na aba nova do mesmo Chrome (mesmos cookies), e a primeira sessão seguiu lendo as rodadas 2 e 3 depois dele. Não foi testado — e continua sendo risco — login do mesmo CPF em **outro** navegador (outra máquina, ou a operadora entrando com o mesmo CPF)

- [x] **Achado na conferência da prova, corrigido e no ar (16:54)**: a consulta do Ian mostrou o item 1 da BAQPLAST como "sem proposta", quando o log o leu em 8º a cada rodada. Causa, no log: às 16:48:22 a rodada 4 começou no mesmo segundo do encerramento, a aba fechou no meio da leitura, a leitura voltou vazia e o estado vazio foi ao Praefectus por cima do 8º lugar. Duas correções: o que é lido depois do fim da sessão não vai ao Praefectus (nem estado, nem lance); e página sem nenhuma proposta legível passa a ser "não sei", não "a empresa não tem proposta". Os outros itens estavam certos no banco: item 2 em 6º (R$ 4.247,70 × 2.940), item 3 em 4º (R$ 5.579,20 × 3.495), item 5 desclassificada; Santa Rosa sem proposta. `session-manager.js` `491fb973…` e `portals/comprasgov.js` `464067ea…`, backups `.bak-20260916-1654`
- [x] **Prioridade de leitura dos itens em disputa** (aprovado pelo Ian em 16/09) — no ar às 16:59. Com algum item em disputa (fase aberta, encerramento aleatório, fechada ou desempate), ele é lido a cada rodada e os que aguardam no máximo a cada minuto; sem item em disputa — e enquanto a fase não é lida da sala, como hoje —, todos a cada rodada, como antes. Motivo: cada item custa alguns segundos de página; 4 itens davam rodada de ~14 s, 40 passariam de 2 minutos, lento demais para a iminência. `itensParaLer` em `estrategia.js` (3 testes) e 1 teste no laço
- [x] **Aba em segundo plano não desacelera** — conferido na VPS que o Puppeteer 22.15 já passa `--disable-background-timer-throttling`, `--disable-backgrounding-occluded-windows` e `--disable-renderer-backgrounding` por padrão (a afirmação anterior, de que o Chrome não tinha nenhuma, estava errada); as três foram escritas no `browser.js` para uma atualização do Puppeteer não as tirar em silêncio. `browser.js` `be6c6331…`, `session-manager.js` `b94c4958…`, `estrategia.js` `82bbe704…`, backups `.bak-20260916-1659`
- [x] **Limite de pregões simultâneos: 6** — decisão do Ian em 16/09 ("se a VPS for travar e ficar pesada e lenta, pode ser 6 ou 7"). A VPS tem 8 GB de RAM e 4 processadores; pela conta de RAM caberiam 14, e o limite configurado era 8. O gargalo é o processador (dar lance na iminência pesa mais que acompanhar, e o vigia abre um Chrome de vez em quando). `MAX_SESSOES_PARALELAS=6` no `.env` da VPS às 17:00 (backup `.env.bak-20260916-1700`), `/health` com `max_sessoes: 6`. O limite é do servidor, dividido entre todos os clientes; cada pregão ocupa uma vaga, em aba ou em Chrome próprio; o 7º é recusado, o agendador tenta de novo a cada minuto (até 5 vezes) e avisa. Medir o uso com pregões reais e subir para 7 se houver folga

- [x] **Incidente de 16/09, ~17:00 — Santa Rosa e BAQPLAST apareceram juntas na tela** (apontado pelo Rafael). Causa: na preparação da prova da BAQPLAST (≈14:55), a conta do Grupo Santa Rosa (`comercial@gruposantarosa.com.br`, dona da credencial do Compras.gov e do agente) foi incluída como `viewer` da BAQPLAST, para a disputa de prova aparecer na tela. Telas que listam pelo que o usuário pode ver (RLS por empresa), e não pela empresa ativa, passaram a mostrar dados das duas. Não houve publicação de código (o domínio seguia na `2026-09-16.2`). Correção, feita pelo Ian no SQL Editor às ~17:15: vínculo removido (a conta voltou a pertencer só à Santa Rosa, como admin desde 30/05); apagados a disputa de prova da BAQPLAST (`9ed940f2`), as duas sessões de teste dela (`3508533b`, `a42e545e`) e os 13 avisos de teste. Os e-mails de teste de 16:23 aos usuários da BAQPLAST já tinham saído. **Regra daqui em diante: teste do robô não mexe em vínculo de usuário com empresa**; prova com empresa de cliente só com disputa e conta da própria empresa

**Fase 5 — liberar o lance**
- [x] **Lance liberado no Compras.gov** — 16/09/2026 às 18:31, **a pedido do Ian** ("pode ligar o robô pra dar lances, mandaram eu ligar"), sobre a autorização de **Giovanny Valente e Rubens** (14–15/09: "pode testar fazer lances sem problemas"). `PORTAIS_COM_LANCE_LIBERADO = ['comprasgov']`, com o registro no comentário; `/health` do servidor e do domínio com `['comprasgov']`. O que segura o lance daqui em diante, nas palavras do Ian, são as configurações e o botão de parar: **modo automático** da disputa, **piso** por item, **teto** de lances, **robô da empresa** ligado, **parada pelo painel** e **freio de emergência**
  - **antes de ligar, dois defeitos corrigidos** (os dois de 27/02/2026, commit do Lovable, e nunca rodados porque a trava estava fechada):
    - o `enviarLance` do Compras.gov tinha seletores escritos antes de alguém ver a sala: sem achar o campo, digitava em **qualquer** campo visível com "valor" ou "proposta" e clicava em **qualquer** botão com "enviar" ou "registrar" — na sala logada, "Registrar intenção de recurso" casa — e aceitava **qualquer** modal com "sim" ou "ok". Agora o lance só sai no campo cujo nome fala de lance, **dentro do bloco da tela que cita o item** (o item 1 não é o 12) e sem campo de outro item, com **o botão desse bloco** ("Enviar lance", "Enviar", "Ofertar") e só confirma modal que fala de lance. Qualquer dúvida — nenhum campo, dois campos, dois botões — **não clica em nada**: avisa uma vez por item ("lance não enviado: campo de lance do item N não encontrado") e segue acompanhando, tentando de novo a cada minuto. Aceite só por frase de lance ("lance registrado", "lance recusado"…), não pela palavra "sucesso" solta; sem frase, o lance **não conta** e a próxima leitura mostra se entrou. 4 testes com a tela montada (dois itens, item 12 × item 1, isca "Registrar intenção de recurso", página pública)
    - o interruptor **"Modo Automático"** do cadastro ("o robô enviará lances automaticamente") **nunca chegava ao robô** — desligado, não seguraria nada. Agora a tela e o webhook mandam o valor (o gravado na disputa vale mais) e o robô só dá lance com **true explícito**; desligado, o quadro diz "Modo automático desligado nesta disputa: o robô acompanha e não dá lance", e o lembrete avisa
  - **no ar**: `robo-lances-webhook` **v50** às 18:31; na VPS às 18:31, `estrategia.js` `f52b7b8c…`, `portals/comprasgov.js` `87aa0009…`, `session-manager.js` `d30056ff…`, `index.js` `c34bbfad…`, md5 iguais ao template, conferido antes que os instalados eram os das 16:59/17:24 (nada de terceiros), backups `.bak-20260916-1831`, 0 sessões, nenhum Chrome órfão, `/health` online
  - **o robô sai quando o portal passou da fase de lances** — escrito e no ar em 16/09 às 19:09 (`estrategia.js` `5ecdfcc7…`, `portals/comprasgov.js` `9f3087b5…`, `session-manager.js` `bb341bbc…`, md5 iguais ao template, conferido antes que os instalados eram os das 18:31, backups `.bak-20260916-1909`, 0 sessões, nenhum Chrome órfão). A fase da sala ainda não é lida, então o robô não percebia o fim e ficava até o limite de **10 horas** (`HORAS_MAXIMAS_SESSAO`), ocupando uma das 6 vagas e recarregando a página. A página pública da compra mostra a **situação de cada item** — no 7/2026, depois da sessão, "Aguardando julgamento". Da segunda rodada em diante, a cada 3 minutos, o laço lê as situações de uma vez (`situacoesDosItensNoTexto`, testada com o texto real) e encerra o item com situação posterior aos lances (julgamento, aceito, adjudicado, homologado, habilitado, recurso, deserto, fracassado, cancelado, revogado, anulado); a sessão encerra quando todos acabam. "Encerramento aleatório" e "Disputa encerrada" (fim da etapa aberta, com a fechada por vir) **não** contam. A primeira rodada sempre lê a sala antes — o quadro mostra a posição da empresa mesmo numa compra já encerrada. 5 testes (regra, texto real, laço)
    - **efeito na demonstração com o 7/2026**: o robô entra, lê a posição de cada item e sai em cerca de 1 minuto com "a fase de lances terminou no portal (situação: Aguardando julgamento)" — antes ficava na sala até alguém parar
  - **o que isso muda hoje, e o que não muda**: o robô lê a sala pela **página pública** de cada item (recarregada a cada rodada), e o campo de lance só existe na **sala logada do fornecedor**, que ainda não foi vista. Então, numa disputa real agora, o robô **decide** o lance pela estratégia e pelo piso e avisa "campo de lance do item não encontrado" — **não envia**. O lance sai de verdade quando a sala logada for mapeada (Fase 1), no pregão que o Rafael e a Izabelle escolherem: abrir a sala logada numa segunda aba para dar lance, com a leitura seguindo na pública
...
**Fase 6 — checklist do grupo** (confirmado pelo Ian em 16/09: é o checklist do Giovanny de 14/09, 20:10; a integração Licitanet por API e o design system clássico ficaram com o Giovanny e não são tocados aqui)

> **A fonte não é o PNCP, e por quê** (conferido ao vivo em 16/09): a consulta
> do PNCP por unidade exige o CNPJ do órgão ("Obrigatório que o CNPJ do órgão
> seja informado") e cada chamada levou ~28 s. Os **dados abertos do
> Compras.gov** acham a compra pelo `idCompra` — UASG (6) + modalidade (2) +
> número (5) + ano (4); o 7/2026 é `92531505000072026` — em ~0,5 s, e os 75
> itens em ~0,9 s, já com as coordenadas do PNCP. A API não libera chamada do
> navegador (sem `Access-Control-Allow-Origin`), então passa por uma edge
> function. Pregão (05), concorrência (03) e dispensa (06) são consultados em
> paralelo, porque o mesmo número pode existir em mais de uma modalidade.

- [x] **Buscar os itens do edital a partir de UASG + número/ano, no cadastro manual** — escrito em 16/09, versão `2026-09-16.9`. Função `compra-comprasgov` (**v1 no ar às 17:41**; sem login responde "Token inválido", CORS liberado) e regra em `_shared/compra-comprasgov.ts`. No passo "Dados da Disputa", com Compras.gov, UASG e número/ano preenchidos, o botão **"Buscar itens e dados no Compras.gov"**: preenche data e horário da sessão (fim do prazo de propostas, com o fuso de Brasília dito na leitura) se estiverem vazios, carrega os itens se a grade estiver vazia — ou oferece "Usar os N itens da compra (substitui os M atuais)" —, e mostra o resumo (modalidade, SRP, modo, critério, órgão, objeto, prazo, quantos itens e se o orçamento é sigiloso) com link para o PNCP. Cada item chega com número, descrição (com "Cota reservada/Participação exclusiva ME/EPP" quando houver), quantidade, unidade, grupo como lote, o **valor estimado pelo órgão** como teto (selo "Compras.gov") e **piso vazio** — piso é decisão da empresa. Compra não encontrada ou API fora do ar aparecem com a mensagem real. 14 testes com a resposta real do 7/2026 (`fixtures/compra-comprasgov-7-2026.json`). **Falta a prova na tela com usuário logado**
  - **orçamento sigiloso** (o 7/2026 inteiro): a API manda valor 0, que não é o valor — o item chega sem valor, e a grade passou a ter o **valor unitário editável** (antes era só leitura, e a disputa não salvava com referência zero). Quem digita assume o número: o selo vira "Manual" e a estimativa do órgão, se havia, segue guardada à parte
  - **defeito achado no caminho e corrigido**: piso e margem da grade comiam a vírgula — cada tecla fazia texto → número → texto, e "4999,70" virava 499970. O campo agora guarda o texto enquanto a pessoa digita (`CampoDecimal`, regra em `lib/robo/valor-digitado.ts`, 4 testes); vale para valor, piso e margem
- [x] **Marca e modelo como colunas editáveis na grade** — coluna "Marca / Modelo" escrita em 16/09: editável em item **sem processo** (manual, edital, Compras.gov), e só leitura em item do processo, com a dica "vêm da Proposta do processo — altere por lá" (a Proposta é a casa desse dado; duas fontes divergiriam). Viajam ao robô no envio (`marca`, `modelo`), para a proposta (Fase 3). Os dados abertos não trazem marca (só a do vencedor, depois do resultado), então o pré-preenchimento vem do anexo (abaixo)
  - **pré-preenchimento pelo termo de referência** — escrito e no ar em 16/09 (função `marca-modelo-do-termo` **v1**, 18:14; versão `2026-09-16.10`). **Conferido antes, com o 7/2026**: o PNCP publicou um arquivo só, "Edital e anexos", **compactado em RAR**, com seis PDFs; o termo ("3. ANEXO B - TR", **112 páginas**) **não indica marca nem modelo** por item — fala em "marca ofertada" e "ou similar". Na Lei 14.133 indicar marca é exceção (art. 41), então o normal é não haver o que preencher, e 112 páginas passam do limite de leitura nativa de PDF da IA (100). Por isso a leitura é **sob demanda**: no passo dos itens, com a compra buscada no Compras.gov, o botão **"Procurar marca e modelo no termo de referência"** lista os arquivos da compra no PNCP, abre PDF, ZIP ou RAR (`node-unrar-js`, WebAssembly), escolhe o termo de referência (depois o edital; minuta e ETP por último), tira o texto e **recorta só os trechos que falam de marca, modelo ou fabricante** — sem trecho, responde sem chamar a IA. Com trecho, a IA recebe só os recortes e a lista de itens e devolve marca/modelo **com o trecho que prova**; o que não se acha no texto é descartado. Preenche **só campo vazio** e só em item sem processo. A tela diz o resultado, inclusive o normal: "Lido: 3. ANEXO B - TR…, 112 páginas. O órgão não indica marca nem modelo nos itens — o normal na Lei 14.133. Preencha com o produto que a empresa oferta."
    - provado localmente com o arquivo real (Deno 2.9): RAR aberto em 0,6 s, termo escolhido primeiro, 112 páginas em texto em 0,55 s, 26 trechos (21 mil caracteres) para a IA. No ar, a função sobe e recusa quem não está logado (401 depois dos imports — as bibliotecas carregaram). **Falta a prova na tela com usuário logado**, que é também a primeira chamada à IA com os recortes
    - 10 testes das regras (`termo-de-referencia.test.ts`: tipo pelos bytes, ordem dos arquivos com os nomes reais, recorte, limite, prova conferida inclusive com o texto de letras soltas do PDF) e 2 da grade (`compra-comprasgov.test.ts`)
    - achado no caminho: `deno check` rodado dentro do repositório lê o `package.json` e troca o `node_modules` do app por links para a pasta do Deno, e mexe no `deno.lock`. Desfeito com `git checkout -- deno.lock` e `npm ci`; checagens com Deno daqui em diante numa cópia fora do repositório
- [x] **Dados da licitação na página da disputa** — escrito em 16/09 (`CompraDaDisputa`): disputa do Compras.gov com UASG e número/ano mostra, abaixo das condições, a compra publicada — modalidade, SRP, modo de disputa, critério, órgão, objeto, prazo de propostas, itens e link do PNCP —, lida ao vivo, sem cópia gravada (a data publicada muda quando o pregão é remarcado). **Avisa quando a data da disputa não confere** com o prazo publicado (sessão fora de 0–60 minutos depois do fim das propostas), ou quando a disputa está sem data: é o "pregão remarcado" para a disputa cadastrada à mão, que o gatilho do processo não alcança. Falha de leitura vira "Tentar novamente" e não bloqueia a página
...
#### 16/09, noite — auditoria: o que as reuniões pediram × o que está no ar

> Pedido do Ian às 22:40: "fizemos tudo que foi levantado nas reuniões sobre esse
> robô? Faça um pente fino". Fontes cruzadas: a reunião de 10/09 com o Rafael (o
> robô integrado ao processo e as quatro expectativas dele), a de 14/09 com o
> Giovanny, o checklist do grupo e as decisões D1–D13. Tudo conferido no código,
> no servidor e nas funções no ar.
...
| D5 — lance autorizado | ✅ liberado às 18:31 · **nenhum lance real ainda**: o campo de lance só existe na sala logada, que não foi mapeada (o robô registra "campo de lance fora da tela" e não clica em nada) |
| D6 — proposta → lance | ⚠️ envio ao robô consertado; o preenchimento do formulário no portal espera a tela numa compra escolhida |
| D7 — entra sozinho e avisa | ✅ entrou, lance recusado, erro, encerrou, lembretes, e-mail · **primeiro lance aceito passou a avisar** (abaixo) · "disputa começou" depende da fase, que ainda não é lida |
| D8 — regras de disputa | ✅ na decisão · **iminência não dá lance no Compras.gov** enquanto o tempo restante não for lido — agora a tela e o lembrete dizem isso · **lance final fechado ganhou campo** (abaixo) |
| D9 — checklist do grupo | ✅ busca testada ao vivo na noite de 16/09 no 7/2026 (75 itens, modo Aberto, SRP, prazos com fuso, 1,1 s); falta a prova na tela logada |
...
**Pente fino do servidor e das funções — sem defeito encontrado:**
- agente online desde a instalação das 19:09, sem erro novo (`error.log` parado em 16:48), 0 sessões, nenhum Chrome órfão, 5,9 GB de RAM livres, disco em 13%;
- os arquivos da VPS têm os md5 da instalação, e o template não mudou depois dela;
- `/health`: lance liberado `['comprasgov']`, certificado carregado, vigia ligado. Uma conferência do vigia falhou às 19:30 (tempo de navegação esgotado) e a seguinte voltou logada; falha de conferência não gera aviso de sessão vencida, só a vencida gera;
- o agendador chamou o `/health` a cada 5 minutos a noite toda. Os picos (15h, 19h, 22h30) são a tela do robô aberta, que consulta a cada 3 s desde 09/09 por causa do código de verificação, e não um laço;
- as funções exigem login, e a ação do agendador sem o segredo responde 401;
- dados abertos do Compras.gov no ar; o 7/2026 ainda não tem resultado publicado, então a conferência de hora em hora não mexe em processo nenhum por ele;
- 387 testes do robô, tipos e lint verdes.

**Feito na mesma noite** (versão `2026-09-16.13`, `robo-lances-webhook` v56; a VPS não mudou, porque o agente já lia o campo):
- [x] **Lance final fechado por item** — campo "lance final R$" na grade, embaixo da estratégia. Aparece quando o modo é aberto e fechado ou ainda desconhecido, e some quando a compra buscada no Compras.gov é de outro modo, a não ser que já tenha valor. Viaja como `lance_final_fechado` no envio manual e no agendador; a página da disputa mostra "final R$ …". A grade avisa lance final abaixo do piso. Teste no laço confirma o nome do campo de ponta a ponta
- [x] **Iminência no Compras.gov** — a grade e o lembrete avisam que, enquanto o tempo restante não é lido, o robô só acompanha nesses itens, e sugerem "Melhor preço" para disputar. O lembrete não repete o aviso quando o lance está travado ou o modo automático está desligado
- [x] **Primeiro lance aceito vira aviso** — por item e por sessão (sininho e canto da tela, levando à disputa). Os lances seguintes ficam na linha do tempo, que passou a registrar o número do item
...
- [x] **Instalado e publicado em 17/09, com OK do Ian**: push `79f90fe2` (origin e sete, remoto conferido parado); na VPS às 02:07 `index.js` (`42dc25e7`), `vigia-sessao.js` (`d2a5e93a`) e `central-notificacoes.js` novo, md5 igual ao template, backups `.bak-20260917-0207`, 0 sessões e 0 Chrome antes de cada `pm2 restart`, `/health` com 16 rotas e lance `['comprasgov']`; a rota sem chave responde 403. `robo-lances-webhook` **v57** às 02:08. Só o `index.js`, o vigia e o módulo novo foram instalados: `callback.js`, `portals/index.js`, `licitanet.js` e os 15 estaduais já diferiam do template antes e não foram tocados
- [~] **Reconhecimento acompanhado — 17/09, 02:09 a 02:31, quatro rodadas.** Cada rodada abriu o perfil guardado (sessão do gov.br ativa, conferida pelo vigia logo antes), foi à Área de Trabalho do Fornecedor e parou sem clicar em notificação nem abrir compra. O módulo foi ajustado entre elas (commits locais, reinstalado a cada vez com md5 e 0 sessões; o final é `central-notificacoes.js` `9a08d925`):
  1. **02:09** — a área só tem 11 links `<a>`, nenhum para o Compras.gov novo. A foto mostra o menu "Dados Cadastrais | Compras | SICAF | Contratos | Sair" desenhado por script;
  2. **02:15** — lendo os scripts: o menu é o "menu132" (Ger Versluis) no quadro `t_top.asp`, com os itens em `MenuN_M = new Array("texto", "endereço")`. Itens de Compras: **"Licitação e Dispensa (novo)" → `/assinadas/dispensa_eletronica.asp`**, "Pregão e Concorrência (legado)" → `/assinadas/pregao.asp`, "Da sua Linha de Fornecimento", "Todas em Andamento", "Em que está Participando (Lances)", "Consultar suas Propostas" e "Consultar Relatórios" → `/assinadas/cotacao.asp`, "RDC Eletrônico", "Linhas de Fornecimento", "Regiões para Notificação"; em Dados Cadastrais, "Controle de Usuários e Opção por Serviços" → `/assinadas/Cadastro_Usuario/controle.asp`. Abrir `dispensa_eletronica.asp` numa aba nova → `accessdenied.htm`;
  3. **02:26** — navegando dentro do quadro `main2` (onde o menu abre as páginas: `DocTargetFrame = "main2"`) → `accessdenied.htm`;
  4. **02:31** — disparando do quadro do menu, como o `GoTo` do `menu132_com.js` faz (`parent.frames["main2"].location.href = BaseHref + endereço`) → `accessdenied.htm`, que diz **"Sua sessão expirou. Clique aqui para logar novamente!"** — com o cabeçalho da mesma tela ainda mostrando empresa e usuário logados.
  
  **Conclusão até aqui:** o caminho existe e está mapeado; o que falta é o portal aceitar a sessão que o robô traz na parte `/assinadas/`. Hipóteses, sem confirmar: o login pelo SSO direto (`authorize` → `landing_sso.asp`, o do robô) não cria a sessão que `/assinadas/` exige, ao contrário do clique humano na tela de login; um cookie de sessão antigo restaurado do perfil guardado conflitando com o novo; ou a opção por serviço do usuário ("Controle de Usuários e Opção por Serviços"). As tentativas pararam em quatro para não gerar acesso automatizado em série na conta de madrugada. A tela remota certa é `https://agente.praefectus.com.br/vnc/vnc.html?path=/vnc/&autoconnect=true&resize=scale` — o `/vnc/vnc_auto.html` fica cinza ("connection is closed"). As fotos da VPS (`logs/screenshots/central-*.png`) mostram CPF e nome do titular: não sair de lá
- [ ] **Próximo passo sugerido**: uma sessão em que uma pessoa clica no menu pela tela remota enquanto o robô só observa e grava as navegações — mostra se o clique humano passa, e por qual endereço
- [ ] **O webhook ainda não alcança o robô para isso**: `avisarNotificacoesDoPortal` usa os segredos `AGENTE_URL_BASE` e `AGENTE_API_KEY`, que **não existem** no projeto (conferido em 17/09 às 02:35 pela lista de segredos; o log do nginx não tem nenhuma chamada a `/notificacoes-portal`). Hoje a função para no começo ("sem agente gerenciado") e não faz nada — inerte, sem efeito na disputa. Ao ligar a integração: ou cadastrar os dois segredos, ou buscar o agente pela linha de `agente_externo_config` (`url_base` + `chaveParaOAgente`), como o envio de sessão faz
- **Conferido para a apresentação de 17/09** (pedido do Ian: "não deixa essa integração atrapalhar"): a leitura da central está desligada no robô (sem `LER_CENTRAL_NOTIFICACOES` no `.env`); o que o vigia faz é o mesmo de antes (as linhas "Central do Compras.gov" no log são só dos quatro reconhecimentos); o agendador segue chamando o `/health` de 5 em 5 minutos; sem erro novo no log do robô; e a parte do webhook é inerte (acima)
...
- [x] **Piso de 10 s entre leituras fora da iminência** (`SEGUNDOS_MINIMOS_ENTRE_LEITURAS`, `estrategia.js`; na iminência a leitura rápida de 3 s continua), com teste; o campo "Intervalo" da disputa passa a ter mínimo 10. **Na VPS às 03:18** (`estrategia.js` `7c6db619`, md5 igual ao template, backup `.bak-20260917-0318`, 0 sessões e 0 Chrome antes do `pm2 restart`; conferido no servidor: intervalo 1 → 10000 ms, 30 → 30000 ms)
...
- [x] **"📺 Assistir o robô ao vivo"** (pedido do Ian): junto do "Robô na sala", um segundo aviso **só para os administradores da plataforma** (a tela remota é compartilhada entre as empresas e não se mostra a cliente), que leva a `/admin/robo-lances?aba=sessoes&tela=abrir` — a página abre a tela remota sozinha e tira o parâmetro da URL. Webhook + `avisoParaAssistirAoVivo`, com testes. **`robo-lances-webhook` v58 às 03:17**
...
#### 17/09, manhã — menu Ações, Detalhes da licitação, remoção com confirmação e tutorial

Pedidos do Ian olhando a `2026-09-17.4` publicada (versão `2026-09-17.8`; a `.6` e a `.7` são do Rafael — pasta do processo às 08:50 e 09:18, monitoramento às 09:52):
