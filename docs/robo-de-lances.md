...
- [~] **Marca e modelo como colunas editáveis na grade** — coluna "Marca / Modelo" escrita em 16/09: editável em item **sem processo** (manual, edital, Compras.gov), e só leitura em item do processo, com a dica "vêm da Proposta do processo — altere por lá" (a Proposta é a casa desse dado; duas fontes divergiriam). Viajam ao robô no envio (`marca`, `modelo`), para a proposta (Fase 3). **Falta** o pré-preenchimento a partir do termo de referência: os dados abertos não trazem marca (só a do vencedor, depois do resultado); depende da extração por IA do anexo
