...
- [ ] **Acompanhamento sem tela remota (D13)** — ordem definida em 16/09: vem antes de "uma aba por pregão"
  ...
  - [ ] prova com uma disputa da BAQPLAST no 7/2026: 8º lugar no item 1, R$ 4.999,70, visível no quadro e nas colunas
    - a BAQPLAST existe no Praefectus (conferido em 16/09) e o robô dela está ligado (sem linha em `robo_empresa_config`), mas **o usuário que é dono da credencial do Compras.gov e do agente não é membro dela**. O agendador tira agente e credencial do dono da disputa, e o CNPJ, da empresa; a tela só mostra a disputa a membro da empresa. Falta decidir a inclusão do usuário como membro da BAQPLAST antes de cadastrar a disputa de prova
