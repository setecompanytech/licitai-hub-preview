...
- [ ] Medir: depois de alguns pregões, contar os desfechos em `logs/logins.jsonl` e registrar aqui quantos logins o perfil guardado evitou e **por quanto tempo** o gov.br mantém a sessão
  - **parcial, 16/09 às 17:26**: 22 linhas no arquivo. Último clique no certificado às **13:31**; depois dele, só `sessao-reaproveitada` (13:46, 14:55, 16:45 ×2, em 2–3 s) e o vigia `logado` a cada 20 minutos até **17:26** — a sessão do gov.br valeu **pelo menos 3h55** sem clique novo, com o vigia passando por ela. Falta ver se atravessa a noite (a primeira disputa da manhã seguinte responde)
...
- [ ] **Vigia: registrar a tela quando der "indefinido"** (pedido do Ian em 16/09: "não esqueça") — gravar em `logs/logins.jsonl` o endereço (sem a parte de parâmetros, que pode carregar token) e se havia captcha na tela, para "indefinido" repetido virar diagnóstico e não palpite. Instalar junto da próxima troca do agente, para não reiniciar a VPS duas vezes
